...
SELECT format(price_in_us_dollars *
    (SELECT us_dollars_to_currency_ratio FROM Currency WHERE currency_name = 'Britsh Pound')
    , '£.00')
	AS 'Price Digital Thermometer in London'
FROM Store_location
...
WHERE store_name = 'London Extension' AND product_name = 'Digital Thermometer'; 

-- 3.
SELECT DISTINCT product_name AS 'Product name', format(price_in_us_dollars *
	(SELECT us_dollars_to_currency_ratio FROM Currency WHERE currency_name = 'Euro')
	, '.00 €')
	AS 'Product price in euros'
FROM Store_location
JOIN Sells ON Sells.store_location_id = Store_location.store_location_id
JOIN Product ON Product.product_id = Sells.product_id
WHERE price_in_us_dollars * 
	(SELECT us_dollars_to_currency_ratio FROM Currency WHERE currency_name = 'Euro') < 26 OR 
	price_in_us_dollars *
	(SELECT us_dollars_to_currency_ratio FROM Currency WHERE currency_name = 'Euro') > 299;

-- 4.
SELECT DISTINCT product_name AS 'Product name', name AS 'Alternate name'
FROM Store_location
JOIN Sells ON Sells.store_location_id = Store_location.store_location_id
JOIN Product ON Product.product_id = Sells.product_id
JOIN Alternate_name ON Alternate_name.product_id = Product.product_id
WHERE product_name IN 
	-- Determines unique products (product names) sold in all locations
	-- ('Bag Valve Mask', 'Digital Thermometer', 'Handheld Pulse Oximeter').
	(SELECT product_name FROM Product JOIN Sells ON Product.product_id = Sells.product_id
	GROUP BY product_name
	HAVING COUNT(Sells.product_id) = -- Sells table contains which product (product_id) is sold in which store location (store_location_id).
		-- Gets count of store locations (5).
		(SELECT COUNT(store_location_id) FROM Store_location)); 

-- 5.
SELECT product_name AS 'Product name', name AS 'Alternate name'
FROM (SELECT Product.product_id FROM Product JOIN Sells ON Product.product_id = Sells.product_id
	GROUP BY Product.product_id
	HAVING COUNT(Sells.product_id) = (SELECT COUNT(store_location_id) FROM Store_location)) results_table_subquery -- Results table of subquery.
-- results_table_subquery includes results table of subquery.
JOIN Product ON Product.product_id = results_table_subquery.product_id 
JOIN Alternate_name ON Alternate_name.product_id = Product.product_id; 
...
JOIN Alternate_name ON Alternate_name.product_id = Product.product_id
WHERE EXISTS (
	-- Gets product_id 101 (Bag Valve Mask), 102 (Digital Thermometer), and
	-- 104 (Handheld Pulse Oximeter).
	SELECT results_table_subquery.product_id FROM
		-- Determines how many stores each product 
		-- (based on product_id) is sold in.
		(SELECT product_id, COUNT(product_id) AS count_stores_sold FROM Sells
		GROUP BY Sells.product_id) results_table_subquery
		WHERE count_stores_sold = 
			-- Gets count of store locations (5).
			(SELECT COUNT(store_location_id) FROM Store_location)
			-- This correlates subquery with outer query.
			AND Product.product_id = results_table_subquery.product_id);

-- 7.
-- Defines view.
CREATE OR ALTER VIEW View_table AS 
SELECT Product.product_id FROM Product JOIN Sells ON Product.product_id = Sells.product_id
	GROUP BY Product.product_id
	HAVING COUNT(Sells.product_id) = (SELECT COUNT(store_location_id) FROM Store_location);

SELECT product_name AS 'Product name', name AS 'Alternate name'
FROM View_table -- Uses view.
JOIN Product ON Product.product_id = View_table.product_id 
JOIN Alternate_name ON Alternate_name.product_id = Product.product_id; 
...
JOIN Product ON Product.product_id = Sells.product_id
JOIN Alternate_name ON Alternate_name.product_id = Product.product_id
JOIN Currency ON Currency.currency_id = Store_location.currency_accepted_id
JOIN Offers ON Offers.store_location_id = Store_location.store_location_id
JOIN Shipping_offering ON Shipping_offering.shipping_offering_id = Offers.shipping_offering_id
ORDER BY price_in_us_dollars ASC;